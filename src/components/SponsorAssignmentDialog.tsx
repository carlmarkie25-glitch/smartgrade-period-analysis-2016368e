...
export const SponsorAssignmentDialog = ({
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Success", description: "Sponsor class assignments updated" });
      queryClient.invalidateQueries({ queryKey: ["sponsor-assignments"] });
      onOpenChange(false);
    },
    onError: (error: any) => {
      toast({ title: "Error", description: error.message || "Failed to update sponsor assignments", variant: "destructive" });
    },
  });
  ...
  if (classes) {
    (classes as any[]).forEach((cls) => {
      ...
      if (!groupedClasses[deptId]) groupedClasses[deptId] = [];
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>{user ? `${user.full_name} - Select classes to sponsor` : "Loading..."}</DialogDescription>
        </DialogHeader>
            ...
            <ScrollArea className="h-[400px] border rounded-lg p-4">
              <div className="space-y-4">
                {Object.entries(groupedClasses).map(([departmentId, deptClasses]) => {
                  ...
                  return (
                    <Collapsible key={departmentId} open={isExpanded} onOpenChange={() => toggleDepartment(departmentId)}>
                      <CollapsibleTrigger className="flex items-center gap-2 hover:bg-accent p-2 rounded-md w-full">
                        ...
                        <Badge variant="secondary" className="ml-auto">
                          {deptClasses?.filter((c) => selectedClasses.includes(c.id)).length}/{deptClasses?.length}
                        </Badge>
                      </CollapsibleTrigger>
                      <CollapsibleContent className="ml-6 mt-2 space-y-2">
                        {deptClasses?.map((cls) => (
                          <div key={cls.id} className="flex items-center gap-2 p-2 hover:bg-accent rounded-md">
                            <Checkbox id={cls.id} checked={selectedClasses.includes(cls.id)} onCheckedChange={() => toggleClass(cls.id)} />
                            <Label htmlFor={cls.id} className="cursor-pointer flex-1 font-normal">
                              {cls.name}
                              <span className="text-xs text-muted-foreground ml-2">({cls.academic_years?.year_name})</span>
                            </Label>
                          </div>
                        ))}
                      </CollapsibleContent>
...
<<<<<<< Updated upstream
      ...
      if (classIds.length > 0) {
        const { error: insertError } = await supabase
          .from("sponsor_class_assignments")
=======
      // Delete all existing assignments using Supabase client (respects RLS and auth)
