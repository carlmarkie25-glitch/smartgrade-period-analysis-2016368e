...
import { X, GraduationCap, UserPlus, Eye, Pencil, Trash2, UserMinus, Search } from "lucide-react";
...
const initialFormState: StudentForm = {
  full_name: "", class_id: "", department_id: "", date_of_birth: "", password: "",
  photo_url: "", phone_number: "", gender: "", nationality: "", ethnicity: "",
  county: "", country: "", religion: "", disability: "", health_issues: "",
  father_name: "", father_contact: "", mother_name: "", mother_contact: "",
  emergency_contact_name: "", emergency_contact_phone: "", emergency_contact_relationship: "",
  previous_school: "", previous_class: "", address: "",
};
...
export const StudentUsersTab = () => {
  ...
  const filteredStudents = useMemo(() => {
    ...
    return students.filter((s: any) => {
      return (
        ...
        (s.departments?.name || "").toLowerCase().includes(term) ||
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>, isEdit: boolean = false) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        if (isEdit) setEditPhotoPreview(reader.result as string);
  ...
  const handleAddStudent = async () => {
    ...
    try {
      ...
      let photoBase64 = "", photoContentType = "";
      ...
      const { data, error } = await supabase.functions.invoke("create-student-account", {
        body: {
          student_id: nextStudentId, password: newStudent.password, full_name: newStudent.full_name,
          class_id: newStudent.class_id, department_id: selectedClass?.department_id || newStudent.department_id,
          date_of_birth: newStudent.date_of_birth || null, phone_number: newStudent.phone_number || null,
          gender: newStudent.gender || null, nationality: newStudent.nationality || null,
          ethnicity: newStudent.ethnicity || null, county: newStudent.county || null,
          country: newStudent.country || null, religion: newStudent.religion || null,
          disability: newStudent.disability || null, health_issues: newStudent.health_issues || null,
          father_name: newStudent.father_name || null, father_contact: newStudent.father_contact || null,
          mother_name: newStudent.mother_name || null, mother_contact: newStudent.mother_contact || null,
          ...
          previous_school: newStudent.previous_school || null, previous_class: newStudent.previous_class || null,
          address: newStudent.address || null, photo_base64: photoBase64, photo_content_type: photoContentType,
        },
      });
  ...
  const handleEditClick = (student: any) => {
    ...
    setEditStudent({
      full_name: student.full_name, class_id: student.class_id, department_id: student.department_id,
      date_of_birth: student.date_of_birth || "", password: "", photo_url: student.photo_url || "",
      phone_number: student.phone_number || "", gender: student.gender || "",
      nationality: student.nationality || "", ethnicity: student.ethnicity || "",
      county: student.county || "", country: student.country || "", religion: student.religion || "",
      disability: student.disability || "", health_issues: student.health_issues || "",
  ...
  const handleUpdateStudent = async () => {
    ...
    try {
      ...
      if (editFileInputRef.current?.files?.[0]) {
        ...
        const { data: photoData, error: photoError } = await supabase.functions.invoke("update-student-photo", {
          body: { student_id: editingStudentId, photo_base64: photoBase64, photo_content_type: contentType },
        });
      ...
      const { error } = await supabase.from("students").update({
        full_name: editStudent.full_name, class_id: editStudent.class_id,
        department_id: selectedClass?.department_id || editStudent.department_id,
        date_of_birth: editStudent.date_of_birth || null, photo_url: photoUrl,
        phone_number: editStudent.phone_number || null, gender: editStudent.gender || null,
        nationality: editStudent.nationality || null, ethnicity: editStudent.ethnicity || null,
        county: editStudent.county || null, country: editStudent.country || null,
        religion: editStudent.religion || null, disability: editStudent.disability || null,
        health_issues: editStudent.health_issues || null, father_name: editStudent.father_name || null,
        father_contact: editStudent.father_contact || null, mother_name: editStudent.mother_name || null,
        ...
        previous_school: editStudent.previous_school || null, previous_class: editStudent.previous_class || null,
        address: editStudent.address || null,
      }).eq("id", editingStudentId);
      if (error) throw error;
      if (editStudent.password) {
        const { data: passwordData, error: passwordError } = await supabase.functions.invoke("update-student-password", {
          body: { student_id: editingStudentId, new_password: editStudent.password },
        });
  ...
  return (
    <div className="space-y-6">
      {/* Student Records with full management */}
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            <CardTitle className="flex items-center gap-2">
              <GraduationCap className="h-5 w-5" />
              Student Users ({students?.length || 0})
            </CardTitle>
            <CardDescription>Manage student accounts, biodata, and records</CardDescription>
          </div>
          <Dialog open={isAddDialogOpen} onOpenChange={(open) => {
            setIsAddDialogOpen(open);
            if (open) getNextStudentId().then(setNextStudentId);
          }}>
