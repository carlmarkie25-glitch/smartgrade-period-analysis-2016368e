...
export const useReportInputs = (studentId: string, period: string, academicYearId?: string | null) => {
  return useQuery({
    queryKey: ["report-inputs", studentId, period, academicYearId ?? "current"],
    queryFn: async () => {
...
export const useSaveReportInputs = (studentId: string, period: string, academicYearId?: string | null) => {
  ...
  return useMutation({
    mutationFn: async (values: ReportInputs) => {
      ...
      const { error } = await supabase
        .from("student_report_inputs" as any)
        .upsert(payload, { onConflict: "student_id,period,academic_year_id" });
      if (error) throw error;
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["report-inputs", studentId, period, academicYearId ?? "current"] });
    },
  });
};
