...
BEGIN
  ...
  -- If the parent rows are already gone (e.g. cascading school cleanup),
  -- skip recomputation. Nothing to update against.
  IF NOT EXISTS (SELECT 1 FROM public.class_subjects WHERE id = v_class_subject_id)
     OR NOT EXISTS (SELECT 1 FROM public.students WHERE id = v_student_id) THEN
